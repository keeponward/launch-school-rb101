...
def gen_uuid()
  uuid_string = ''
  SECTIONS.each_with_index do | num_hex_chars, i |
    num_hex_chars.times do
      uuid_string << HEX_ARR.sample()
    end
    # Write hyphen only if not last section
    uuid_string << '-' unless i >= (SECTIONS.size - 1) 
  end
  uuid_string
end
